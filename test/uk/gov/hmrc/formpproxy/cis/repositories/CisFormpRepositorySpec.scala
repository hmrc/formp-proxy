...
final class CisFormpRepositorySpec extends SpecBase {
  ...
  "deleteVerification" - {
    "delete verification and update version number" in {
      val db   = mock[Database]
      val conn = mock[Connection]
      ...
      when(conn.prepareCall(eqTo(CisStoredProcedures.CallDeleteVerification))).thenReturn(csDelete)
      ...
      repo.deleteVerification(req).futureValue

      verify(csDelete).setString(1, "abc-123")
      verify(csDelete).setLong(2, 77L)
      verify(csDelete).execute()

      verify(conn).prepareCall(eqTo(CisStoredProcedures.CallGetScheme))
      verify(csGetScheme).execute()

      verify(conn).prepareCall(eqTo(CisStoredProcedures.CallDeleteVerification))

      verify(rsScheme).close()
      verify(csGetScheme).close()
      verify(csDelete).close()
    }
  }
