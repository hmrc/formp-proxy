...
@ImplementedBy(classOf[NovaFormDataServiceImpl])
...
@Singleton
class NovaFormDataServiceImpl @Inject() (repo: NovaSource) extends NovaFormDataService {

  override def getFormData(formId: Long, formDataIds: Seq[String]): Future[Option[FormDataResponse]] =
    repo.getFormData(formId, formDataIds)
  ...
  override def getNovaNotificationRef(
    ...
    repo.getNovaNotificationRef(formId, request)
}
