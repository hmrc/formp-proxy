...
class VerificationService @Inject() (repo: CisMonthlyReturnSource) {
  ...
  def deleteVerification(request: DeleteVerificationRequest): Future[DeleteVerificationResponse] =
  ...
  def proceedVerification(request: ProceedVerificationRequest): Future[Unit] =
    repo.proceedVerification(request)
}
