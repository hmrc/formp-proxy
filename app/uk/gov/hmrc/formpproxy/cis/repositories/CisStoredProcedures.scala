...
object CisStoredProcedures {
  ...
  val CallUpdateGetGovTalkStatusCorrelationId = "{ call SUBMISSION_ADMIN.UpdateGovTalkStatusCorr(?, ?, ?, ?, ?) }"
  ...
  val CallGetLastVerificationBatch            =
    "{ call VERIFICATION_PROCS.Get_Last_Verification_Batch(?, ?, ?, ?, ?, ?) }"
