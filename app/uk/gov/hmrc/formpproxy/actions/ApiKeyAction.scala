...
trait ApiKeyAction extends ActionBuilder[Request, AnyContent]

class DefaultApiKeyAction @Inject() (
  configuration: Configuration,
  val parser: BodyParsers.Default
)(implicit val executionContext: ExecutionContext)
    extends ApiKeyAction {
  ...
  override def invokeBlock[A](request: Request[A], block: Request[A] => Future[Result]): Future[Result] =
    request.headers.get("X-API-Key") match {
      case Some(apiKey) if apiKey == expectedApiKey => block(request)
